...
@implementation FNNavgation
FNSingletonM(FNNavgation)
...
+ (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated replace:(BOOL)replace
{
    if (!viewController) {
        ...
        if([viewController isKindOfClass:[UINavigationController class]]) {
            [FNNavgation setRootViewController:viewController];
        } // 如果是导航控制器直接设置为根控制器
        else {
            UINavigationController *navigationController = [FNNavgation sharedFNNavgation].currentNavigationViewController;
            if (navigationController) { // 导航控制器存在
                ...
                [FNNavgation sharedFNNavgation].applicationDelegate.window.rootViewController = navigationController;
...
+ (void)presentViewController:(UIViewController *)viewController animated: (BOOL)flag completion:(void (^ __nullable)(void))completion
{
    if (!viewController) {
        ...
        UIViewController *currentViewController = [[FNNavgation sharedFNNavgation] currentViewController];
        if (currentViewController) { // 当前控制器存在
            ...
            [FNNavgation sharedFNNavgation].applicationDelegate.window.rootViewController = viewController;
        }
    }
}
...
+ (void)popTwiceViewControllerAnimated:(BOOL)animated {
    [FNNavgation popViewControllerWithTimes:2 animated:YES];
}
...
+ (void)popToRootViewControllerAnimated:(BOOL)animated {
    UIViewController *currentViewController = [[FNNavgation sharedFNNavgation] currentViewController];
    NSUInteger count = currentViewController.navigationController.viewControllers.count;
    [FNNavgation popViewControllerWithTimes:count-1 animated:YES];
}
...
+ (void)dismissViewControllerWithTimes:(NSUInteger)times animated: (BOOL)flag completion: (void (^ __nullable)(void))completion {
    UIViewController *rootVC = [[FNNavgation sharedFNNavgation] currentViewController];
