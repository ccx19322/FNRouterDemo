...
@implementation UIViewController (FNRouter)
...
+ (UIViewController *)initFromURL:(NSURL *)url withQuery:(NSDictionary *)query fromConfig:(NSDictionary *)configDict
{
    ...
    NSString *home;
    if (url.scheme == nil) {
        home = [NSString stringWithFormat:@"%@", url.path];
    }
    else if(url.path == nil){ // 处理url,去掉有可能会拼接的参数
        home = [NSString stringWithFormat:@"%@", url.host];
    }else{
        home = [NSString stringWithFormat:@"%@%@", url.host, url.path];
    }
    if([configDict.allKeys containsObject:url.scheme]){ // 字典中的所有的key是否包含传入的协议头
        ...
        id config = [configDict objectForKey:home]; // 自定义的url情况
        Class class = nil;
        if([config isKindOfClass:[NSString class]]){
            class =  NSClassFromString(config); // 根据key拿到对应的控制器名称
            if (class == nil) { // 兼容swift,字符串转类名的时候前面加上命名空间
                NSString *spaceName = [NSBundle mainBundle].infoDictionary[@"CFBundleExecutable"];
                class =  NSClassFromString([NSString stringWithFormat:@"%@.%@",spaceName,config]);
            }
        }
        if(class != nil){
            VC = [[class alloc]init];
